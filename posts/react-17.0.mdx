...
In version 17.0 we saw deprecation warnings, related with some renamed functions, that will be removed in next MAJOR version.
...
Lifecycle function is called right before mutations are made (e.g. before the DOM is updated).
...
class Example extends React.Component {
    ...
    // remove listeners/timers created
    componentWillUnmount() {}

    // create elements that will be render in DOM.
    render() {}
}
```
...
There wasn’t a big change between version 16.8 and version 17.0, was **zero new features**.
